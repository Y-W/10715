...
from theano.scalar.basic import Sgn, same_out_nocomplex, exp, abs_
from theano.tensor.elemwise import Elemwise

sgn_pg_threshold = 1.0

class Sgn_PG(Sgn):
    def grad(self, inputs, gout):
        (x,) = inputs
        (gz,) = gout
        return [gz * (x <= sgn_pg_threshold) * (x >= -sgn_pg_threshold)]
...
sgn2_pg = Elemwise(theano.scalar.sgn2_pg)
